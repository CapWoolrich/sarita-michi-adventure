...
const SIZE = 88;
const MAP_RANGE = 30; // m del jugador que cubre el radar
 ...
 */
export default function MiniMap({ getRadarSnapshot }) {
  ...
  // Convertir mundo (x, z) a radar (x, y) en coordenadas del SVG
  const half = SIZE / 2;
  const blips = cats
    .map((c) => {
      ...
      if (dist > MAP_RANGE) return null;
      // mapear a radar
      const rx = (dx / MAP_RANGE) * (half - 8);
      const ry = (dz / MAP_RANGE) * (half - 8);
