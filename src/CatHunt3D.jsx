...
export default function CatHunt3D() {
  ...
  return <div>
    ...
    {DEBUG_INPUT && <pre data-game-ui="true" style={{ position:'fixed', left:8, bottom:8, zIndex:99, background:'rgba(0,0,0,.72)', color:'#fff', padding:8, fontSize:11 }}>{JSON.stringify({ runtimeCatsLength: runtime.cats.length, runtimeTotalCats: runtime.totalCats, visibleCats, capturedCatIds: runtime.capturedCatIds, capturedCatIdsLength: runtime.capturedCatIds.length, renderedCatIds: runtime.cats.filter((c) => !runtime.capturedCatIds.includes(c.id)).map((c) => c.id), catPositions: runtime.cats.map((c) => ({ id: c.id, position: c.position })), nearestCatId: runtime.nearestCatId, nearestCatDistance: runtime.nearestCatDistance, lastCatchResult: runtime.lastCatchResult }, null, 2)}</pre>}
  </div>;
}
